...
let selectOption = 'allUsers';
const select = document.querySelector('#filterByUsername');
...
const selectOptions = () => {
  for (let i = 0; i < usersFromLocalStorage.length; i++) {
    ...
    userOption.value = usersFromLocalStorage[i].username;
    userOption.innerHTML = usersFromLocalStorage[i].username;
...
document
  .querySelector('#filterByUsername')
...
function show() {
  const postToShow = allPosts
    .filter((post) => {
      ...
      if (selectOption === findUserById(post.ownerId).username) {
        return true;
      }
    })
    .filter((post) => {
      const isThisMyPost = loggedAs.id === post.ownerId;
      if (showMyPosts && selectOption === 'allUsers') {
